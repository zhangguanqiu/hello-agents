...
class CostOptimizationTeam:
    ...
    async def run_auto_gen_team_chat(self, task: str, max_iterations: int = 3):
        ...
        print(f"   每轮包含：成本分析 → 稳定性评估 → 安全评估 → 最终决策")
        print(f"   如有问题，成本智能体将根据反馈调整方案\n")
...
4. **成本智能体**：根据评估结果输出最终建议"""
                ...
                for msg in result.messages:
                    if hasattr(msg, 'source') and hasattr(msg, 'content'):
                        ...
                        if json_data:
                            ...
                            elif source == 'ProductManager':
                                product_manager_output = json_data
            ...
            if product_manager_output:
                if 'recommended_instance' in product_manager_output:
                    recommendation_lines.append(f"推荐机型: {product_manager_output['recommended_instance']}")
                if 'recommendation' in product_manager_output:
                    recommendation_lines.append(f"推荐原因: {product_manager_output['recommendation']}")
                if 'recommended_operation' in product_manager_output:
                    recommendation_lines.append(f"推荐操作: {product_manager_output['recommended_operation']}")
            ...
            if product_manager_output:
                print(f"\n💰 成本智能体推荐:")
                if 'recommended_instance' in product_manager_output:
                    print(f"   机型: {product_manager_output['recommended_instance']}")
                if 'recommended_operation' in product_manager_output:
                    print(f"   操作: {product_manager_output['recommended_operation']}")
                if 'recommendation' in product_manager_output:
                    # 截断过长的推荐原因
                    reason = product_manager_output['recommendation']
                    if len(reason) > 100:
                        reason = reason[:100] + "..."
                    print(f"   原因: {reason}")
            
            # 显示稳定性评估
            print(f"\n🔧 稳定性智能体评估: {'✅ 通过' if engineer_approved else '❌ 不通过'}")
            if stability_reason:
                # 截断过长的原因
                reason = stability_reason if len(stability_reason) <= 150 else stability_reason[:150] + "..."
                print(f"   {reason}")
            
            # 显示安全评估
            print(f"\n🔒 安全智能体评估: {'✅ 通过' if code_reviewer_approved else '❌ 不通过'}")
            if security_reason and not code_reviewer_approved:
                # 只在不通过时显示详细原因
                reason = security_reason if len(security_reason) <= 150 else security_reason[:150] + "..."
                print(f"   {reason}")
            
            # 显示最终决策
            print(f"\n⚖️  最终决策: {'✅ 允许变更' if final_decision_allow else '❌ 拒绝变更'}")
            ...
            if final_decision_allow:
                all_passed = True
                print(f"\n{'='*60}")
                print(f"🎉 第 {iteration} 轮评估全部通过！方案已被采纳。")
                print(f"{'='*60}")
                break
            else:
                iteration += 1
...
async def run_optimized_software_development_team():
    ...
    print("✓ 智能体团队创建完成")
    print("   - 成本智能体 (ProductManager)")
    print("   - 稳定性智能体 (Engineer)")
    print("   - 安全智能体 (CodeReviewer)")
    print("   - 决策智能体 (FinalDecisionAgent)")
...
84核320G的IT5.21XLARGE320价格11541.6"""
    ...
    if result['all_passed']:
        ...
        if result['failure_reasons']:
            print(f"\n❌ 未通过的原因:")
            # 去重并显示
            unique_reasons = list(dict.fromkeys(result['failure_reasons']))
            for i, reason in enumerate(unique_reasons, 1):
                print(f"   {i}. {reason}")
        
        print(f"\n💭 建议:")
        print(f"   1. 考虑调整业务需求或负载")
        print(f"   2. 咨询专业技术团队")
        print(f"   3. 评估其他配置方案")
